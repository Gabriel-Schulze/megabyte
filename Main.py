...
class Main:
    ...
    def criandoWidgets(self):
        ...
        self.frame2 = Frame(self.frame1,background="#223a9c")
        self.frame2.grid(column=1,row=1,rowspan=2)
        self.frame2.config(width=210,height=810)
        self.frame2.pack_propagate(False)
        
        
        self.frameAdm = Frame(self.frame2,background="#223a9c")
        ...
        CTkLabel(self.frameAdm,text_color="#ffffff", text=sys.argv[2],font=("Open Sans",19)).grid(column=2,row=0)
            
        
        self.inicioLink = CTkLabel(self.frame2,text_color="#ffffff",font=self.fontMenu ,text="Inicio")
        self.inicioLink.grid(column=1,row=1, columnspan=2,pady=(150,20))
        self.inicioLink.bind("<Button-1>", lambda e: self.telaInicio())
        
        if self.user == "admin":
            self.usuarioLink = CTkLabel(self.frame2,text_color="#ffffff",font=self.fontMenu ,text="Usuario")
            self.usuarioLink.grid(column=1,row=2, columnspan=2,pady=(10,20))
            self.usuarioLink.bind("<Button-1>", lambda e: self.telaUsuario())
        
        self.produtoLink = CTkLabel(self.frame2,text_color="#ffffff",font=self.fontMenu ,text="Produtos")
        self.produtoLink.grid(column=1,row=3, columnspan=2,pady=(10,20))
        self.produtoLink.bind("<Button-1>", lambda e: self.telaProduto())
        
        self.categLink = CTkLabel(self.frame2,text_color="#ffffff",font=self.fontMenu ,text="Categorias")
        self.categLink.grid(column=1,row=4, columnspan=2,pady=(10,20))
        self.categLink.bind("<Button-1>", lambda e: self.telaCategoria())
        
        self.fornLink = CTkLabel(self.frame2,text_color="#ffffff",font=self.fontMenu ,text="Fornecedores",width=210)
        ...
        self.frameConfig = Frame(self.frame2,background="#223a9c")
        ...
        self.configLink = CTkLabel(self.frameConfig,text_color="#ffffff",font=self.fontMenu ,text="Configurações")
        self.configLink.grid(column=2,row=1)
        
        CTkLabel(self.frame2,text_color="#ffffff",text="Version 1.0",anchor=S).grid(column=1,row=7,sticky=W,padx=5)
        self.btn_Logoof = CTkLabel(self.frame2 ,text="LogOff",font=("Open Sans",18),text_color="red")
